...
const DoctorVisits = () => {
  ...
  const handleDelete = async (visitId) => {
    const confirmDelete = window.confirm("आपण ही भेट खरोखर हटवू इच्छिता?");
    if (!confirmDelete) return;

    try {
      await axios.delete("/api/Docter/DocterVisits", {
        data: { visitId },
      });
  ...
  return (
    <div className="gradient-bg min-h-screen p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl font-bold mb-6 text-center text-white bg-blue-600 p-3 rounded-lg shadow-lg">
          डॉक्टर भेटी
        </h2>
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-3 ">
                    ...
                    <div className="flex items-start">
                      ...
                      <p className="text-gray-700">{visit.status}</p>
                    </div>

                  <button
                    onClick={() => handleDelete(visit._id)}
                    className="mt-4 w-full bg-red-500 text-white text-sm py-2 rounded hover:bg-red-600 transition"
                  >
                    हटवा
                  </button>
                </div>
