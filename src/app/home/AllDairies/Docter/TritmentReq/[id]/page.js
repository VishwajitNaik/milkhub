...
export default function TritmentReq() {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchVisits = async () => {
            try {
                const res = await axios.get(`/api/sangh/DocterVisit/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`/api/sangh/completeVisit/${selectedVisit._id}`, {
                ...formData,
                doctorId: id
            });
